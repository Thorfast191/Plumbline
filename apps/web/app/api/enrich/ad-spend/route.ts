...
import { parseAdSpendCsv, upsertAdSpendRows } from "@plumbline/enrich";
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!csv.trim()) {
    return Response.json({ error: "Request body is empty — expected CSV text (header: channel,date,spend_minor,currency_code)." }, { status: 400 });
  }

  let parsed: ReturnType<typeof parseAdSpendCsv>;
  try {
    parsed = parseAdSpendCsv(csv);
  } catch (err) {
    return Response.json({ error: err instanceof Error ? err.message : String(err) }, { status: 400 });
  }

  const { count } = await upsertAdSpendRows(prisma, store.accountId, store.id, parsed.rows);
